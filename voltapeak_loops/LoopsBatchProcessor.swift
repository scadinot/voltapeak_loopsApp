...
enum LoopsBatchProcessor {
    ...
    /// Crée/nettoie le dossier de sortie : supprime les PNG/CSV/XLSX résiduels.
    static func cleanOutputFolder(_ folder: URL) {
        let fm = FileManager.default
    ...
    static func processOne(url: URL, options: BatchOptions) -> BatchFileResult {
        ...
        guard let meta = FileNameParser.parse(fileName) else {
            return BatchFileResult(url: url, fileName: fileName, status: .skipped, analysis: nil)
        }

        do {
            let raw = try SWVFileReader.readFile(at: url, config: options.config)
            let (potentials, currents) = SWVFileReader.processData(raw)
            ...
            return BatchFileResult(url: url, fileName: fileName,
